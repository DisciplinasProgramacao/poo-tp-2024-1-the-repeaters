...
namespace VegTable
{
    ...
    public class Restaurante : Estabelecimento
    {
        ...
        public string FazerReserva(string nomeReserva, int numPessoas)
        {
            ...
            if (cliente == null)
            {
                throw new Exception("Cliente não cadastrado. Por favor, cadastre o cliente antes de fazer a reserva.");
            }
        ...
        public override void FazerPedido(string idReserva, string ids)
        {
            ...
            if (reserva == null)
            {
                throw new Exception("Reserva não encontrada ou pedido já fechado.");
            }

            if (string.IsNullOrEmpty(ids))
            {
                throw new Exception("IDs de produtos inválidos.");
            }

            List<int> idsProdutos = ids.Split(',').Select(id => int.Parse(id.Trim())).ToList();
            List<Produto> produtosAdicionados = Cardapio.GerarPedido(idsProdutos, reserva.Pedido.Itens);
        ...
        public Reserva FinalizarReserva(string idReserva, int numPessoasFinalizar)
        {
            ...
            if (numPessoasFinalizar <= 0 || numPessoasFinalizar > reserva.QuantPessoa)
            {
                throw new Exception("Número de pessoas inválido.");
            }
            ...
            if (reserva != null)
            {
                ...
                double valorPorPessoa = reserva.Pedido.CalcularTotal() / numPessoasFinalizar;
        ...
        public string MostrarListaDeEspera()
        {
            ...
            {
                throw new Exception("A lista de espera está vazia.");
            }
        ...
        public string MostrarClientesNasMesas()
        {
            ...
            {
                throw new Exception("Nenhum cliente está nas mesas.");
            }
